...
parser.add_argument('--mask1',type=str,default='data/JiNan_City/mask1.csv',help='mask1 path')
parser.add_argument('--poi',type=str,default='data/JiNan_City/POI_data_300.csv',help='mask3 path')
parser.add_argument('--poi_vec',type=str,default='data/JiNan_City/Poi.npy',help='mask3 path')
...
def main():
    ...
    transmit_np=np.float32(np.loadtxt(args.mask1,delimiter=','))
    transmit=torch.tensor(transmit_np).to(device)
    poi_vec=np.load(args.poi_vec)
    poi=np.loadtxt(args.poi,delimiter=',')
    poi_1=(poi)/(np.std(poi,-1,keepdims=True)+0.00001)
    
    poi_1=np.float32(poi_1)
    poi_1=torch.tensor((poi_1)).to(device)
    poi_vec=np.float32(poi_vec)
    poi_vec=torch.tensor((poi_vec)).to(device)
    
    masks=[transmit,poi_1,poi_vec]
    print(args)
    if args.model=='gwnet':
    ...
    elif args.model=='Pgwnet':
        engine = trainer17( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                         ,masks)
    elif args.model=='MMgwnet':
        engine = trainer16( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                         ,masks)
    ...
    elif args.model=='PASTGCN_Recent':
        engine = trainer19( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                        ,masks
                         )
    elif args.model=='MMASTGCN_Recent':
        engine = trainer18( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                        ,masks
                         )
    ...
    elif args.model=='MMSTGCN':
        engine = trainer22( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                         ,masks)
    elif args.model=='PSTGCN':
        engine = trainer23( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                         ,masks)
    ...
    elif args.model=='POGCRNN':
        engine = trainer25( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                         ,masks)
    elif args.model=='MMOGCRNN':
        engine = trainer24( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay
                         ,masks)
        ...
        engine = trainer12( args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.decay,masks
                         ) 
