...
class Cross_M(nn.Module):
    def __init__(self,c_in,tem_size,num_nodes):
        super(Cross_M,self).__init__()
        self.conv4=(Conv2d(c_in,64, kernel_size=(1,tem_size),padding=0,
                          stride=1, bias=True))
        
        self.linear2=(torch.nn.Linear(50, 64, bias=True))
        self.linear3=(torch.nn.Linear(64, 16, bias=True))
        self.linear4=(torch.nn.Linear(64, 16, bias=True))
        self.linear5=(torch.nn.Linear(16, 8, bias=True))
        self.linear6=(torch.nn.Linear(16, 8, bias=True))
        
        self.conv5=(Conv2d(64+16+8,32, kernel_size=1,padding=0,
                          stride=1, bias=True))#32
        self.conv6=(Conv2d(32,1, kernel_size=1,padding=0,
                          stride=1, bias=True))#32
        self.bn1=BatchNorm2d(32,affine=True)#32
        self.bn2=BatchNorm2d(1,affine=True)#32
        self.ln3=LayerNorm([16],elementwise_affine=True)
        
        self.linear1=nn.ModuleList()
        for i in range(8):
            self.linear1.append(torch.nn.Linear(16, c_in, bias=True))
        
        self.gate_p=( nn.Conv1d(in_channels=64,
                                    out_channels=64,
                                    kernel_size=1,
                                    bias=False)    )
        self.num_nodes=num_nodes
        self.tem_size=tem_size
    def forward(self,poi_vec,x,poi_1):
        shape=x.shape
        
        poi_vec1=torch.sigmoid(self.linear2(poi_vec))
        poi_vec2=torch.sigmoid(self.linear3(poi_vec1))
        poi_vec3=torch.sigmoid(self.linear5(poi_vec2))
        
        #x=x.permute(0,1,3,2).contiguous().view(shape[0],-1,shape[2]).permute(0,2,1)
        
        x_1=torch.sigmoid(self.conv4(x).squeeze().permute(0,2,1))
        x_2=torch.sigmoid(self.linear4(x_1)) 
        x_3=torch.sigmoid(self.linear6(x_2))
        
        fusion11=(torch.abs(x_1.unsqueeze(-2)-poi_vec1.unsqueeze(0)))
        fusion21=(torch.abs(x_2.unsqueeze(-2)-poi_vec2.unsqueeze(0)))
        fusion31=(torch.abs(x_3.unsqueeze(-2)-poi_vec3.unsqueeze(0)))
        
        fusion_1=torch.cat((fusion11,fusion21,fusion31),-1).permute(0,3,1,2)
        
        dist1=self.conv5(fusion_1)
        dist1=self.bn1(dist1)
        dist1=self.conv6(dist1)
        #
        dist1=self.ln3(dist1)
        
        dist1=torch.sigmoid(dist1).permute(0,2,3,1)
        
        poi=poi_1.reshape(self.num_nodes,16,1,1)
        poi=poi*(dist1.unsqueeze(-2))
        poi=poi.reshape(shape[0],self.num_nodes,-1)
#        # poi2=self.poi.repeat(shape[0],1,1)
#         poi3=poi+poi_1
       
#         gate=(torch.sigmoid(self.gate_p(poi3.permute(0,2,1))).permute(0,2,1))
#         poi=gate*poi+(1-gate)*poi_1

        poi1=[]
        for i in range(8):
            poi1.append(self.linear1[i](poi))
        poi=torch.stack(poi1,-1)
        poi=torch.mean(poi,-1)
        poi=torch.sigmoid(poi)
        
        poi=poi.permute(0,2,1).unsqueeze(-1)
        return dist1,poi

class Cross_P(nn.Module):
    def __init__(self,c_in,tem_size,num_nodes):
        super(Cross_P,self).__init__()
    ...
    def forward(self,poi_vec,x,poi_1):
        ...
        poi1=[]
        for i in range(8):
            poi1.append(self.linear1[i](poi_1))
        poi=torch.stack(poi1,-1)
        poi=torch.mean(poi,-1)
        poi=torch.sigmoid(poi)
        
        poi=poi.permute(1,0).unsqueeze(-1)
        return poi,poi
